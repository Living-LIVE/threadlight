...
Shy is the default. Ordinary messages produce no provider calls and no response. Slash commands,
the Ask Threadlight message action, and direct mentions enter the shared per-conversation queue
immediately.

## Medium

Medium treats conversation as a burst:
...
High queues every eligible human text message and produces exactly one response under the normal
operating envelope. High guarantees engagement, not a Scripture quotation. Discernment may
...
starting another provider request. High mode should be used only in a dedicated channel or
thread.
...
High mode also requires `confirm_high: True`.

Medium and High send recent conversation context to the configured AI provider without an
